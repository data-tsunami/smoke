...
Web interface to execute Scala jobs in Spark. The output generated by `spark-shell` while it's been generated (thanks to websockets). Requires passwordless connection to the cluster using `ssh` (for launching the job). Uses `spark-shell` in `yarn client` mode.
